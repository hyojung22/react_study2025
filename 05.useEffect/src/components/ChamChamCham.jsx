...
const ChamChamCham = () => {
    ...
    const handleClick = (selectUser) => {
        // 컴퓨터는 랜덤한 값이 나오게 하려고 랜덤 함수로 돌림
        const selectCom = select[Math.floor(Math.random() * select.length)];
        // 변한 값 상태변수에 넣어주기
        setUser(selectUser);
        setCom(selectCom);
    };
    ...
    return (
        <div>
            ...
            */}
            ...
            <button onClick={() => handleClick('좌')}>좌</button>
            <button onClick={() => handleClick('정면')}>정면</button>
            <button onClick={() => handleClick('우')}>우</button>
        </div>
    )
}
